...
class NormConvTransposeBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, pixelnorm=True, activation=True):
        super(NormConvTransposeBlock, self).__init__()
...
class netG_nvidia_Generic(nn.Module):
    def __init__(self, args):
        ...
        if self.use_upsampling:
            layers.append(Upsample(scale_factor = 4))
            layers.append(NormConvBlock(self.nz, count*self.ngf, kernel_size=3, stride=1, padding=1, pixelnorm=True))
        else:
            layers.append(NormConvTransposeBlock(self.nz, count*self.ngf, kernel_size=4, stride=1, padding=0, pixelnorm=True))
        if self.extra_cap:
        ...
        while count > 1:
            ...
            if self.use_upsampling:
                ...
                layers.append(NormConvTransposeBlock(2*count*self.ngf, count*self.ngf, kernel_size=4, stride=2, padding=1, pixelnorm=True))
            if self.extra_cap:
                layers.append(NormConvBlock(count*self.ngf, count*self.ngf, kernel_size=3, stride=1, padding=1, pixelnorm=True))
...
class netD_nvidia_Generic_Cap(nn.Module):
    def __init__(self, args):
        ...
        # layers.append(PixelNormLayer())
        self.linear1 = self.Linear(count*self.ndf, 1024)
        self.main = nn.Sequential(*layers)
    ...
    def __init__(self, args, n_extra_layers=0):
        ...
        for t in range(n_extra_layers):
            ...
            if self.norm_type == 'instance':
                main.add_module('extra-layers-{0}_{1}_instancenorm'.format(t, cndf),
        ...
        while csize > 4:
            ...
            if self.norm_type == 'instance':
                main.add_module('pyramid_{0}_instancenorm'.format(out_feat),
...
class DCGAN_G(nn.Module):
    def __init__(self, args, n_extra_layers=0):
        ...
        if self.norm_type == 'instance':
            main.add_module('initial_{0}_instancenorm'.format(cngf),
                                nn.InstanceNorm2d(cngf))
        else:
        ...
        while csize < isize//2:
            ...
            if self.norm_type == 'instance':
                main.add_module('pyramid_{0}_instancenorm'.format(cngf//2),
                                nn.InstanceNorm2d(cngf//2))
            else:
        ...
        for t in range(n_extra_layers):
            ...
            if self.norm_type == 'instance':
                main.add_module('extra-layers-{0}_{1}_instancenorm'.format(t, cngf),
                                nn.InstanceNorm2d(cngf))
            else:
        ...
        main.add_module('semifinal_{0}-{1}_convt'.format(cngf, cngf),
                        nn.ConvTranspose2d(cngf, cngf, 4, 2, 1, bias=False))
        if self.norm_type == 'instance':
            main.add_module('semifinal_{0}_instancenorm'.format(cngf), nn.InstanceNorm2d(cngf))
        main.add_module('semifinal_{0}_relu'.format(cngf), nn.ReLU(True))
        main.add_module('prefinal_{0}-{1}-conv1x1'.format(cngf, cngf//2), nn.Conv2d(cngf, cngf//2, 3, 1, 1, bias=False))
        if self.norm_type == 'instance':
            main.add_module('prefinal_{0}_instancenorm'.format(cngf//2), nn.InstanceNorm2d(cngf//2))
        main.add_module('prefinal_{0}_relu'.format(cngf//2), nn.ReLU(True))
        main.add_module('final_{0}-{1}-conv1x1'.format(cngf//2, nc), nn.Conv2d(cngf//2, nc, 3, 1, 1, bias=False))
