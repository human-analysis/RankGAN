...
class Trainer():
    def __init__(self, args, modelD, modelG, Encoder, criterion, prevD, prevG):
        ...
        self.params_loss_train = ['Loss_D0', 'Loss_G0', 'Score_D0', 'Score_D1', 'Score_D0_G0', 'Score_D0_G1', 'Score_D1_G1', 'Disc_Diff', 'Disc_Acc']
        ...
        self.params_monitor_train = ['Loss_D0', 'Loss_G0', 'Score_D0', 'Score_D1', 'Score_D0_G0', 'Score_D0_G1', 'Score_D1_G1', 'Disc_Diff', 'Disc_Acc']
    ...
    def setup_stage(self, stage, dataloader=None):
        if stage == 0:
            ...
            # self.lr_dis = self.lr_dis*1e-1
            # self.lr_gen = self.lr_gen*1e-1
        ...
        if stage == 0:
            ...
            while i < self.len_dataset:
                ...
                while j < d_iterations and i < self.len_dataset:
                    ...
                    # print("data loading time", time.time() - time1)
                    ...
                    net_error.backward(retain_graph=True)
                    ...
                    if j < d_iterations:
                        ...
                        % (stage, epoch, self.stage_epochs[stage], i, self.len_dataset, lossD, lossG, mean_scores_D0, mean_scores_D0_G0, disc_acc, gradient_penalty.item()))
                        ...
                        if i % self.plot_update_interval == 0:
                            ...
                            losses_train['Real'] = input[:64].cpu()
                            losses_train['Fakes'] = fake_G0[:64].detach().cpu()
                            losses_train['Fakes_Previous'] = torch.zeros(fake_G0[:64].size())
                            self.visualizer_train.update(losses_train)
                            # print("data showing time", time.time() - time2)
                ...
                self.monitor_train.update(losses_train, batch_size)
                print('Stage %d: [%d/%d][%d/%d] Loss_D: %.3f Loss_G: %.3f Score_D0: %.3f Score_D0_G0: %.3f Disc_Acc %.3f GP %.3f'
                % (stage, epoch, self.stage_epochs[stage], i, self.len_dataset, lossD, lossG.item(), mean_scores_D0, mean_scores_D0_G0, disc_acc, gradient_penalty.item()))
                ...
                if i % 250 == 0:
                    try:
                        ...
                        if len(input.size()) < 3:
                            ...
                            vutils.save_image(fake_normal_z.data[:64], '%s/fake_samples_stage_%03d_epoch_%03d_normal_z.png' % (self.args.save, stage, epoch), normalize=True)
            ...
            while i < self.len_dataset:
                ...
                while j < d_iterations and i < self.len_dataset:
                    ...
                    net_error = (scores_D0_G1 - scores_D0).mean() + errD0
                    ...
                    if j < d_iterations:
                        ...
                        # losses_train['Clamp'] = self.clamp_upper
                        self.monitor_train.update(losses_train, batch_size)
                        print('Stage %d: [%d/%d][%d/%d] Loss_D: %.3f Loss_G: %.3f Score_D0: %.3f Score_D0_G0: %.3f Score_D0_G1: %.3f Disc_Acc %.3f GP %.3f'
                        % (stage, epoch, self.stage_epochs[stage], i, self.len_dataset, lossD, lossG, mean_scores_D0, mean_scores_D0_G0, mean_scores_D0_G1, disc_acc, gradient_penalty.item()))
                        ...
                        if i % self.plot_update_interval == 0:
                            ...
                            losses_train['Real'] = input[:64].cpu()
                            losses_train['Fakes'] = fake_G0[:64].detach().cpu()
                            losses_train['Fakes_Previous'] = fake_G1[:64].detach().cpu()
                            self.visualizer_train.update(losses_train)
                ...
                while j < self.g_iter and i < self.len_dataset:
                    ...
                    if j < self.g_iter:
                        ...
                        # losses_train['Clamp'] = self.clamp_upper
                        self.monitor_train.update(losses_train, batch_size)
                        print('Stage %d: [%d/%d][%d/%d] Loss_D: %.3f Loss_G: %.3f Score_D0: %.3f Score_D0_G0: %.3f Disc_Acc %.3f Disc_Diff %.3f GP %.3f'
                        % (stage, epoch, self.stage_epochs[stage], i, self.len_dataset, lossD, lossG, mean_scores_D0, mean_scores_D0_G0, disc_acc, disc_abs_diff.item(), gradient_penalty.item()))
